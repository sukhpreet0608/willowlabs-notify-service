...
public class SmsNotificationConsumer {
    ...
    @Bean
    public Consumer<Notification> smsConsumer() {
        return this::processNotification;
    }

    @Transactional
    public void processNotification(Notification notification) {
        log.info("Received SMS notification with id={}", notification.getId());

        try {
            // Send push
            smsHandler.handle(notification);
            ...
            log.info("SMS notification {} processed and DB updated", notification.getId());

        } catch (Exception ex) {
            log.error("Failed to process SMS notification {}", notification.getId(), ex);
